...
| Surface | Files | What it is | Hosted by |
|---|---|---|---|
| **Stream view (overlaid cards)** | [`src/workspace/StreamView.tsx`](../src/workspace/StreamView.tsx) (406 lines), [`src/design/overlaid-cards/OverlaidCards.tsx`](../src/design/overlaid-cards/OverlaidCards.tsx) | State container + presentational layout for the left-to-right stack of navigation/focus "cards" (up to 4 visible, ancestor breadcrumbs as faded edge tabs). The workspace plugin's one face. | "Workspace" tab in the view switcher. |
...
### The overlaid-cards exploration (`src/design/overlaid-cards/`)
...
- Defines its own root-level custom properties that shadow/duplicate the token system rather than importing it: `--card-void: hsl(230, 20%, 7%)` (duplicated from overlaid-cards), `--text-primary: #d8d8e4`, `--text-dim: #8888a0`, `--text-muted: #5a5a70`, `--accent: #7c8cf8` (a *different* blue-violet from the canonical `--c-accent`).
...
| `src/design/overlaid-cards/` | 0% against canonical tokens (self-consistent parallel system) |
