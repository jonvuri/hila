...
- **Two write tiers:** *recognized SQL → editable cells* (a sound recognizer; update-only, single-base-table for v1 — the *view-update problem*), and *arbitrary SQL → read-only*. The `SQLITE_ENABLE_COLUMN_METADATA` capability spike (Session 1) came back **negative** — the bundled `sqlite-wasm@3.50.1` lacks `sqlite3_column_origin_name`, so S2 takes the **AST-parsing** route over the known dialect, reusing the `sqlite3-parser` machinery that already backs tables-visited invalidation (see Phase-9.3.md).
...
- **Build plan:** **S1 read slice — DONE** (bands table + read-only `QueryBand` in the focus panel + minimal SQL authoring) → S2 recognized-SQL write-back → S3 (later) schema-aware editor polish.
