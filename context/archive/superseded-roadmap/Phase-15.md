...
- **No offline queue:** The sync engine requires a network connection to the provider. If the provider is unreachable, changes accumulate locally in the changelog. When the connection resumes, the engine uploads the accumulated changeset. There is no separate offline queue -- the changelog _is_ the queue.
