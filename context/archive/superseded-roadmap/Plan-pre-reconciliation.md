...
> **Superseded.** This file mixes the historical Phase 1–10 plan, obsolete implementation
> descriptions, and the unstarted Phase 11–16 roadmap. Use the active [roadmap](../../Plan.md). The
> replacement decisions are preserved in
> [phase-boundary reconciliation](../phases/Phase-Boundary-Reconciliation.md#proposed-roadmap).
...
> Detailed task list: [Phase-1.md](../phases/Phase-1.md)
...
> Detailed task list: [Phase-2.md](../phases/Phase-2.md)
...
> Detailed task list: [Phase-4.md](../phases/Phase-4.md)
...
> Detailed task list: [Phase-4b.md](../phases/Phase-4b.md)
...
> Detailed task list: [Phase-5.md](../phases/Phase-5.md)
...
> Follow-on exploration: Phase 7b (overlaid-cards polish, [Phase-7b.md](../phases/Phase-7b.md)) and Phase 7c (row–table continuum design exploration, [Phase-7c.md](../phases/Phase-7c.md)). The 7c exploration was promoted into the implementation phases below: its data layer is Phase 8, its view layer is Phase 9, and the design-system pass that follows is Phase 10.

---

### Phase 8 -- Data layer: the ownership spine

> Detailed task list: [Phase-8.md](../phases/Phase-8.md) (umbrella + Part A), [Phase-8b.md](../phases/Phase-8b.md) (derived projections), [Phase-8c.md](../phases/Phase-8c.md) (matrix ownership + tags-as-nodes)

The data-layer realization of the [Phase 7c](../phases/Phase-7c.md) ownership-centric resolution. The largest structural change since the foundation: it promotes the `own`-join from "a cross-matrix lifecycle link" to **the universal tree edge**, dissolves the per-matrix `rank` trait (order moves onto the edge as a sibling-local key), rebuilds `closure` and the scroll index as **derived global caches** over the own-forest, unifies cascade deletion, and reduces tags to nodes + ownership. Split into three sequential, session-sized parts:

- **Part A -- the own-edge foundation.** `own` becomes the tree edge with a sibling-local order key; a root sentinel gives every row exactly one inbound `own`-edge; `insertRow`/`reparentRow`/delete are rewritten onto edges (a reparent becomes an O(1) edge re-point). The standalone `rank` trait dissolves.
- **Part B -- derived projections and unified cascade.** Global identity-keyed `closure` cache + a global pre-order **scroll index** that keeps windowed scrolling a single keyset range scan while interleaving heterogeneous cross-matrix siblings; one cascade over `own`-descendants.
- **Part C -- matrix ownership and tags-as-nodes.** A thin `matrix.owner` fact; own-matrix = `own`-edges to a matrix's roots + the owner fact; the tag registry dissolves into named, promotable **type-nodes**; "tag" splits into label (`ref`) vs type (`own`). The promotion taxonomy falls out.

**Proves:** One ownership mechanism (`own`-edges) expresses outline nesting, tags, collections, and sub-matrices identically, intra- and cross-matrix. Hierarchy is the edge; reachability and scroll order are rebuildable caches. Resolves `Plan.md` open question #3 (create-vs-link = own-vs-ref) and narrows #4. Pre-alpha with no live data makes the rewrite acceptable -- the database is treated as disposable.

---

### Phase 9 -- View layer: rendering the row–table continuum

> Detailed task list: [Phase-9.md](../phases/Phase-9.md)

The rendering & interaction half of the continuum, unlocked by the Phase 8 data model. The 7c exploration's view-layer sub-phases (7c.1–7c.5) plus the resolution's rendering hand-off: heterogeneous children in one outline (multi-table hydration gather), the **property surface** (intrinsic columns ∪ owned aspect fields, realizable by Phase 11's renderer registry), embedded collections & live (query-bound) views with editable-in-place write-back, dedicated sub-table embedding (the old `row_kind = 1` stub reinterpreted as a view-layer position marker), boundary-hop rendering with the panel stack keyed by `(matrix_id, row_id)`, a unified creation gesture, and paradigm convergence as a forward note.

**Proves:** Outline and table are faces over the same own-forest, composable and swappable. Resolves `Plan.md` open question #5 (face affinity).

---

### Phase 10 -- App structure and cohesive design system

> Detailed task list: [Phase-10.md](../phases/Phase-10.md)

A planning-first design pass (renumbered from the original Phase 7d). Settles the overall view hierarchy (is the workspace the root or a peer?), the plugin view-composition model, and a single cohesive design-token/theming system that reconciles the canonical design language with the overlaid-cards exploration -- then sequences the incremental `global.css` migration. Most of its early output is documentation and resolved decisions; it follows Phases 8–9 so the token system spans the final set of surfaces (property surface, embedded collections, query views, cross-matrix navigation). The view hierarchy and navigation model is settled — see [resolved design decision #8](#resolved-design-decisions) and [Architecture - View hierarchy and navigation](../../Architecture.md#view-hierarchy-and-navigation). A comprehensive context audit and reorganization is [planned after Phase 10](../phases/Phase-10.md#post-phase-10-documentation-consolidation-planned-and-deferred), once the moving architecture and design work no longer make a broad rewrite counterproductive.

---

### Phase 11 -- Tasks and movie reviews (tag aspects in practice)
...
> Detailed task list: [Phase-11.md](Phase-11.md)

---

### Phase 12 -- Scheduling infrastructure and notifications
...
- **Timed prompt system.**
  ...
  - Uses the scheduler from Phase 12 to fire prompts at configured intervals.
...
> Detailed task list: [Phase-15.md](Phase-15.md)
> Detailed specification: [Sync.md](../../Sync.md)
...
### Phase 16 -- Op system, batch ops, and MCP server
...
- **Deterministic performance guards.** Beyond building with performance in mind, hot paths are protected by **deterministic tests** rather than noisy wall-clock measurements: query-plan guards (`EXPLAIN QUERY PLAN` asserting index use, no full scans, no automatic indexes), work-count complexity guards (statements / rows / cache-rows touched per op), scaling-ratio tests (work grows at the expected order from N to kN), and reactive invalidation fan-out guards (an edit recomputes only overlapping subscriptions). This practice is established by [Phase 8](../phases/Phase-8.md#performance-testing-strategy-shared-across-parts-abc) (the data-layer ownership spine, which makes the closure/scroll-index/`joins` caches global) and applies to all later performance-sensitive work. Wall-clock benchmarks, if used, are coarse non-gating backstops.
...
- Phase 11+: Context-specific shortcuts for each face type.
...
Phase 3 (Sync-readiness)
  ...
  ├───────────────────────────┐
  ▼                           ▼
Phase 4 (Plugins, Faces,    Phase 12 (Scheduling
...
Phase 11 (Tasks +             │
  ...
  ├──────────────────┐
  ▼                  ▼
Phase 13 (SRS)    Phase 14 (Journaling)

Phase 15 (Live sync, files, Dropbox) ◄── Phase 3
  (can proceed any time after Phase 3;
   independent of Phases 4-14)
```

Phase 3 (sync-readiness) is a prerequisite for everything that follows -- all data changes should be sync-tracked from the start. It establishes unique IDs, change tracking, and the changeset/conflict layer but does not include live sync or file storage. Phase 15 (live sync, files, Dropbox) can proceed any time after Phase 3 and is independent of phases 4-14; it is placed late because the feature work benefits from having more content and use cases before investing in live sync. Phases 4 and 12 can proceed in parallel after Phase 3. Phase 4 (complete) formalized the plugin system, built the face slot model, introduced trait auto-provisioning, and delivered the notes plugin with wiki-links alongside the refactored outline. Phase 4b retrofits the wiki-link implementation with the generalized inline reference system, adds `ref`/`own` join kinds with cascade deletion to the core, and adds reference-type cells to the table face. Phase 5 (tags) builds on the inline references plugin's `#` mode to provide tag type management and the tag-specific UX. Phase 5b (column identity and schema integrity) follows Phase 5 -- it introduces stable column IDs, column constraints, FK-backed column references, and formula column references. It addresses gaps exposed by Phase 5's tag registry migration and should land before Phase 11, whose predefined tag type columns (status, due_date, priority) are the first heavy users of plugin schema contracts. Phase 6 (column display roles) is a small core addition that prepares for Phase 7 and future search. Phase 7 (workspace plugin and stream view) merges the outline and notes plugins into a single workspace with the stream view face, replacing the separate Outline/Notes/Notes Outline tabs. Phase 7b adds overlaid-cards follow-ups (clickable ancestor tabs, slimmed navigation chrome, a Storybook design surface, and a collapsed-breadcrumb theme concept). Phase 7c is a design-exploration pass on the row–table continuum: it settles the conceptual model (a single "attachment" primitive unifying tags, sub-tables, and live query views, over a data/view-layer split) and was promoted into the implementation phases that follow. Phase 8 (data layer: the ownership spine, split into Parts A/8b/8c) realizes 7c's ownership-centric resolution -- `own` as the universal tree edge, rank-on-edge, derived global closure/scroll-index caches, `matrix.owner`, and tags-as-nodes; it requires Phase 7 and reshapes the structures Phase 11 builds on, so it lands before further tag work. Phase 9 (view layer) renders the continuum surfaces unlocked by Phase 8 (property surface, embedded collections/live views, dedicated sub-tables, boundary-hop navigation) and depends on Phase 8. Phase 10 (app structure and cohesive design system, renumbered from the original 7c→7d) is a planning-first pass that settles the view hierarchy, the plugin view-composition model, and a single design-token/theming system; it follows Phases 8-9 so the token system spans their new surfaces. Phase 11 (tasks and movie reviews as tag aspect types) requires Phase 5b and the Phase 8c type-node model. Tasks ship initially without reminders; task reminders are added once Phase 12 lands. Phases 13 and 14 both require the scheduling infrastructure from Phase 12 and can proceed in parallel after it. Phase 16 (op system, batch ops, MCP server) can proceed any time after Phase 5b -- it formalizes the op registry that already exists, adds the batch executor and Markdown I/O layer, and ships the MCP server. Earlier is better: agent access benefits all subsequent phases.
...
7. **Matrixes remain independent at the top level.** Matrixes are not nested inside a single root tree. They live in a flat registry and can be optionally placed in the outline. Plugin housekeeping matrixes remain as background infrastructure accessible through their identity faces and the matrix browser, not forced into the user's workspace. _Refined by the [Phase 7c](../phases/Phase-7c.md) ownership resolution ([Phase 8c](../phases/Phase-8c.md)):_ a node owns a matrix via `own`-edges to its root rows plus a thin `matrix.owner` fact; the old `row_kind = 1` "child matrix reference" carries position, not ownership, and is reinterpreted as a view-layer marker ([Phase 9](../phases/Phase-9.md)). The tag registry dissolves into named, promotable type-nodes rather than a registry matrix.

8. **View hierarchy and navigation model.** Everything at the view layer is a **place** (a position in the one forest), a **gesture** (`⌘K` go / `/` make — transient, never places), or the **system edge** (settings, dev tools — kept tight to what lives above the database). The stream is the single primary surface with one literal root: ancestry up to the root is never hidden, re-rooting is the existing _focus_ mechanic (no "zoom" primitive), and the early top-level tabs are retired — their content is expressible as focus states and launcher lenses. Matrixes are the _membership_ plane, exposed as lenses (`view` nodes + launcher dimension filters), never a second root. Cross-cutting management dissolves into the _views find · faces edit · commands make_ triad. Ancestry resolves provenance → home → membership (an arbitrary portal is never auto-chosen). Placeless creation homes by provenance. Plugins contribute **face types** (each declaring up to two renderings — _line_ and _collection_; the panel scaffold stays shell-owned) and **commands** (one registry, `/` and `⌘K` surfaces); the shell is fixed core infrastructure, and composition follows the _subject → affinity ladder → host_ contract (session 3, [Plugins.md — Plugin view composition model](../../Plugins.md#plugin-view-composition-model), resolving open question #5). URLs/deep-linking deferred; stream state stays serializable. See [Architecture - View hierarchy and navigation](../../Architecture.md#view-hierarchy-and-navigation) and [Phase 10 §2](../phases/Phase-10.md#2-view-hierarchy-and-navigation-model).

9. **Query spec and SQL-analog gestures.** One authoring model behind the `⌘K` launcher's filtered search, persisted `view` nodes/blocks, and result-surface gestures: **gestures edit a spec (kind · scope · text · where · order · limit); the spec compiles to canonical SQL; SQL is the only stored/executed form (preserving `executed == stored`); a recognizer lifts dialect SQL back into the spec** — recomputed on read, so there is no conversion moment and no one-way door inside the dialect. Compiled views are always `SELECT d.*` + `id` from a single base table (editable by construction; projection is a permanent spec non-goal). Escape hatches are per-clause opaque leaves — fragment (WHERE), **CTE source** (FROM, with recursive updatability lift), computed (SELECT) — so one exotic term never evicts a view from gesture-land. Chips are the state: object-as-rendered-app-wide + op glyph + value, name-only input, sigils only as family narrowers, and a menu → typeahead → typed-operator mastery ladder. Save-search-as-node = compile + store + provenance homing. See [Query-Spec.md](../../Query-Spec.md) and [Phase 10 §3b](../phases/Phase-10.md#3b-launcher-deep-dive--the-query-spec) (determinations D1–D19).
...
2. **Full-text search timing.** FTS5 index over matrix content, guided by column display roles (Phase 6). Deferred for now -- the ProseMirror JSON supports simple text extraction, so the index can be added later without schema changes. Revisit when the workspace has enough content to warrant it (likely Phase 11+).

3. **Singleton tags and shared aspects.** _Resolved by the [Phase 7c](../phases/Phase-7c.md) ownership resolution ([Phase 8c](../phases/Phase-8c.md)): create-vs-link **is** own-vs-ref._ `#`-tagging that creates a new aspect row is an `own`-edge; tagging/linking an existing entity (and the no-schema label case) is a `ref`-edge. The model now expresses both cleanly via the two edge kinds, so this is no longer open. _Still holds under the [Phase 9.7 convergence](../phases/Phase-9.7.md#5-portals-and-refs-one-family-split-by-anchoring):_ `#`/`@` keep their 8c semantics (because ownership stays where-created); the convergence only _adds_ a third non-owning relation — the structural **portal** — completing the ownership × anchoring square without disturbing create-vs-link.

4. **Labeled/typed joins.** The join table carries `kind` (`ref`/`own`) but no role or label (e.g. "author", "assignee"). _Narrowed by the ownership resolution:_ `own`-edges are typed by their target matrix, so labels buy nothing there; only `ref`-edges want optional labels for richer reverse lookups ("appears as Author in..."). Deferred until a backlink/reverse-lookup view needs it.

5. ~~**Face affinity for matrixes.**~~ **Resolved ([Phase 10 §3](../phases/Phase-10.md#3-plugin-view-composition-model), session 3):** the **preferred face is a subject-level (matrix-level) rendering recipe** — rung 2 of the composition **affinity ladder** (appearance override [modeled but unstored in v1] → subject/matrix preferred recipe → substrate floor). It is set by the plugin at creation or by the user through panel/block chrome, and rides the membership plane so it follows a container everywhere it appears. The **substrate remains the universal fallback** (rung 3), confirming the [Phase 9.2](../phases/Phase-9.2.md) floor answer and the [Phase 9.7](../phases/Phase-9.7.md#9-open-decisions-and-their-resolutions) substrate-first stance. See [Plugins.md — the affinity ladder](../../Plugins.md#the-affinity-ladder-resolves-which-face-renders-a-subject). _(Original framing: when you drill into an aspect/record row or an embedded matrix, which face should render it? A "preferred face" annotation on a matrix would let plugins specify a default over the table identity face.)_
