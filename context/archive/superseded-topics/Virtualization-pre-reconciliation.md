...
> **Status: requires current-contract rewrite.** Production uses bounded multi-window pagination
> over the global `scroll_index`, SQL-side collapse, gathered block slices, and plural portal
> positions. Earlier rank-era and single-window sections below are migration history, not current
> architecture. Proposed Phase 11 will promote the shipped contract and archive the chronology.
