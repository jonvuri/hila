...
Three layers, bottom to top (full detail in [`context/Architecture.md`](../Architecture.md)):
...
- **Traits** — per-matrix structural metadata tables the core provisions on demand (`ensureTrait(type, matrixId)`), shared by all consumers, persistent. Two exist:
  - **rank** — Lexorank tree-position ordering. _Global_ table partitioned by `matrix_id`; the Lexorank key encodes both order and hierarchy (a parent's key is a prefix of its children's).
...
- **Join table** — _global_ infrastructure (not a trait). Cross-matrix row references with a `kind`:
...
- **Child-matrix references are stubbed.** The `rank` schema has `row_kind` with `0 = row`, `1 = child_matrix_ref` (where `row_id` then holds the _child matrix's_ id) — see `src/core/matrix.ts` line ~80. **Nothing creates a `row_kind = 1` entry**, and `FocusPanel` only renders a placeholder string: _"Child matrix reference (row_kind=1). Table face would render here."_ (FocusPanel.tsx ~line 463).
...
**This is a design-exploration phase, not a feature build.** Output is documentation and resolved decisions, then a sequence of focused follow-on sub-phases — _before_ implementation. It's the row/table analog of the 7/7b breadcrumb deep dives. Full text: [`context/Phase-7c.md`](./phases/Phase-7c.md).

### 4.1 The settled foundation (don't relitigate unless you have strong reason)

**Two layers — separate how structure is _stored_ from how it is _seen_:**

- **Data layer (stored):** everything is rows in matrixes. A row carries structure in its own columns or _attaches_ structure stored in another matrix.
...
- **Thesis:** "homogeneous vs heterogeneous" is a _storage_ distinction; "outline vs table" is a _viewing_ distinction. They are orthogonal and composable.
...
- **own-rows @ 0..N vs own-matrix:** do a scoped collection over a _shared_ matrix and a _dedicated_ matrix feel different enough to warrant distinct gestures, or do they unify in the UX? (revisit in 7c.2 / 7c.3)

### 4.4 Coordination with other phases & cross-cutting open questions

- **Phase 8 is the same mechanism.** Phase 8's `#task` / `#movie-review` aspect rows via `createDependentRow` **are** the own-rows binding; their identity faces are table faces over the tag-type matrix. Phase 8's renderer registry, tag templates, and tag property panel are the **rendering substrate** for the 7c.1 property surface. Design 7c.1 so the Phase 8 registry/panel can realize it directly. (Phase 8 detail: [`context/Phase-8.md`](./phases/Phase-8.md).)
- **Phase 7d** (later renumbered Phase 10) is the cohesive design-system / view-hierarchy / theming pass. It deliberately **follows** 7c so the token system spans 7c's new surfaces (property surface, embedded collections, query views, cross-matrix nav). Its big open questions: is the workspace the root of all views or a peer? full set of top-level views + navigation? plugin view-composition model? one reconciled design language? ([Phase 10](phases/Phase-10.md).)
...
Canonical reading order at the time was `Phase-7c.md` → `Architecture.md` → `Traits.md` → `Plugins.md` → `Plan.md` → `Phase-8.md` and the later Phase 10.
