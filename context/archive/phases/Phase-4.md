...
Concrete tasks for Phase 4. See [Plan.md](../../Plan.md) for context and objectives, [Plugins.md](../../Plugins.md) for the plugin model and face slot system, and [Traits.md](../superseded-topics/Traits-pre-reconciliation.md) for the provisioning model.
...
- [x] Implement `unregisterPlugin(db, pluginId)`:
  ...
  - Does NOT delete matrixes or traits (per [Traits - Provisioning model](../superseded-topics/Traits-pre-reconciliation.md#provisioning-model): "traits survive plugin removal"; matrixes are core entities).
...
Decouple trait creation from matrix creation and implement the `ensureTrait` provisioning API per [Traits - Provisioning model](../superseded-topics/Traits-pre-reconciliation.md#provisioning-model). Currently, `createMatrix` always creates a closure table, and rank entries are created implicitly by `insertRow`. The new API makes trait provisioning explicit, lazy, and shared.
...
- [x] Define the outline plugin definition in `src/outline/outline-plugin.ts`:
  ...
  const outlinePlugin: PluginDefinition = {
    ...
    namedQueries: {
      /* outline page query, breadcrumbs, etc. */
    },
    namedMutations: {
      /* insertRow, reparentRow, deleteRow wrappers */
    },
    faceBindings: [
      {
        key: 'main',
    ...
    init: async (ctx) => {
      /* seed welcome row if matrix is new */
    },
  }
  ```
...
- [x] Define the notes plugin in `src/notes/notes-plugin.ts`:
  ...
  const notesPlugin: PluginDefinition = {
    ...
    init: async (ctx) => {
      /* create default welcome note if matrix is empty */
    },
  }
  ```
