...
- [x] **Add client function** in `matrix-client.ts`:
  ...
  export const updateColumnRole = (
    ...
    columnName: string,
    role: 'label' | 'content' | null,
  ): Promise<void> => matrixOp('updateColumnRole', { matrixId, columnName, role })
  ```
...
- **Roles are orthogonal to slots.** Roles describe what a column _means_ in the matrix's conceptual model (its identifying label, its body content). Slots describe where a column _renders_ in a particular face. The workspace face (Phase 7) uses roles to locate its data columns; it could also use slot bindings, but roles provide a face-independent semantic layer. A column might have role `'label'` and be bound to a slot named `'label'` in the workspace face and to slot `'primary_content'` in the outline face -- the role is stable, the slot binding varies by face.
