...
Add the `kind` column to the join table schema and migrate existing data. See [Traits - Join kinds](../superseded-topics/Traits-pre-reconciliation.md#join-kinds).
...
- [x] Add `createRefJoin` as an alias for `insertJoin` with `kind = 'ref'` (explicit API per [Traits - Core operations](../superseded-topics/Traits-pre-reconciliation.md#core-operations)). Added in both `matrix.ts` (core) and `matrix-client.ts` (client).
...
Implement the core lifecycle rules for `own`-kind joins per [Traits - Lifecycle rules](../superseded-topics/Traits-pre-reconciliation.md#lifecycle-rules).
