...
> retired, and one **substrate** renderer draws every row. The _anchoring_ axis and the
...
dilemma both dissolve once you stop solving for _aspect rows specifically_ and solve
...
- **Source / query** — _which rows are in the set_ (the `WHERE`). Endpoints we have
...
most one owner ([Traits.md — Join kinds](../superseded-topics/Traits-pre-reconciliation.md#join-kinds)); "1 vs. N" is just a
count per query. Even the "merge fields into the focal node" case can be plural: a node
whose prose says "…then #task email the vendor and #task file the report" owns _two_
...
What the group-vs-reorder dilemma was _really_ about is **where the edge's source of
truth lives** — call it _anchoring_. It is independent of presentation and it has a
...
| -------------- | ---------------------------------------------------------- | --------------------------------------------- | ----------------------------------------------------------- |
| **unanchored** | none — a node-scoped query; rows are foreign               | no line                                       | meaningless (you edit the _query_, not an edge)             |
...
| **content**    | an `own`-join materialized from an inline `#`-ref in prose | crosses the border, terminates _on the token_ | the token sits in human prose that may be phrased around it |

Tiers `structural` and `content` are _both_ `kind = 'own'` joins — the data layer does
...
([Traits.md — Semantics](../superseded-topics/Traits-pre-reconciliation.md#semantics), and rule 2 of
[Lifecycle rules](../superseded-topics/Traits-pre-reconciliation.md#lifecycle-rules): deleting the inline `#` removes the
`own` join and cascades the target).

**Anchoring is not "edge strength."** It does not forbid moves. A content-anchored
aspect is _mechanically_ re-parentable — splice the inline ref out of the source prose
...
carries a prose-coherence judgment the system cannot make. That is a _human_ judgment,
...
1. **Merged** — a bound record's fields render as inline properties _of the focal node_
   ...
   actually is: a left-join `host ⋈ aspect` on the own-edge — a _presentational_ join,
   not a data fusion.
...
The group-vs-reorder dilemma is just _banded_ (clarity, predictable gestures, table
affordances) vs. _meshed_ (composability, emergent structure). **It is resolved by not
deciding globally:** default owned aspects to _banded_ (matches their content/structural
...
- An **unanchored** (query) set cannot _mesh_ into the focal node's owned order and
  cannot _merge_ as editable properties of _this_ node — its rows aren't owned by it. It
  can only sit _banded_.
...
focal node's owned order (they share an _anchor domain_); query bands can be visually
...
_(Open: the exact fold/merge gesture vocabulary and how compatibility is surfaced — see
Open sub-questions.)_
...
- **Content anchor** → manipulate the `#`-token in the text: cut/paste/delete the tag
  ...
  The tag _is_ the handle; the banded/meshed row is a projection of it. Cross-node moves
...
the _degenerate_ face: every column binds to a labeled cell, and _all_ relationship
...
`integration`; it reveals _merged_ as a left-join (so merge semantics survive being shown
...
- **§9.1 heterogeneous children** = a _meshed_ band with the outline face; meshing is
  gated by anchoring.
- **§9.2 property surface** = the _merged_ level (intrinsic ∪ content-anchored aspect
  fields) **plus** the _aspect band_ + the schema-adaptive renderer + the tether. The
  "0/1" cardinality language is dropped.
- **§9.3 embedded collections / live views** = _query bands_ (unanchored): no tether, a
...
- **Fold/merge gesture vocabulary.** ~~The exact gestures for fold/unfold/merge and how
  compatibility (anchor domain + face) is surfaced to the user.~~ **Closed by [Phase 9.7](Phase-9.7.md#5-portals-and-refs-one-family-split-by-anchoring): fold/merge is retired** (only collapse remains); the _merged_ level moves to [Phase 10](Phase-10.md).
