...
- **aspect band** (`AspectBand.tsx`, later removed) — editable hydrated
...
> Visual (see [Phase-9.7-visuals.html](../visuals/Phase-9.7-visuals.html)): the matrix border bounds
...
- New renderer + `loose`/`container`/`view` block rendering replacing the three bands
  (`AspectBand`, later removed/[QueryBand](../../../src/workspace/QueryBand.tsx)/[SubTableBand](../../../src/workspace/SubTableBand.tsx)).
...
- [Traits.md — Join / Anchoring](../superseded-topics/Traits-pre-reconciliation.md#join) — add the **portal** (non-owning, structural,
...
1. **Consolidation (this doc + [Phase-9.7-visuals.html](../visuals/Phase-9.7-visuals.html)).** Done.
...
> plus the deltas below and is your blueprint (it is _not_ wired into production ops; that's
...
- **Multi-location maintenance ([scroll-index.ts](../../../src/core/scroll-index.ts)).** New
  ...
  the fresh-attach path inserts one row per parent appearance _directly_ (no recursive
...
> (segment _sizes_, not counts, drive boundaries) and a straddling window gathers exactly the
...
- **`usePagedWorkspaceData` seam.** With markers excluded from the loose scan, the production loose
  ...
  the window-layer seam in the hook and belong with **Stage C** (block _content_ rendering is renderer
...
- **Deferred to Stage C (not Stage B):** folding block _content_ inline into the outline (feeding real
...
> face](../../Architecture.md#hydration) and [Traits.md — Join](../superseded-topics/Traits-pre-reconciliation.md#join). Read the three band
> components being replaced — `AspectBand.tsx` (later removed),
...
> collapse. The _merged_ level (fields inline on a node) stays **Phase 10** composed sugar,
...
- **Position-keyed outline (`pk` = hex `global_lexkey`).**
  ...
  The row _renderer_ (the PM editors) is otherwise untouched — that is C2.
- **Ghost tombstones.** `buildPaginatedOutlineQuery` projects `is_ghost`; NavigationPanel renders
  ...
  drag, no drill. **Subtlety worth carrying to C2:** the ghost guard must be a _reactive_
...
- **C2 inherits:** route NavigationPanel's loose outline rows through the substrate renderer
  ...
  fold `view`/`container` block _content_ inline via the Stage-B `gatherWindow` + the worker↔client
...
- **C2 remainder — inline block folding via the worker↔client gather RPC (landed).** A `view`
  block's _content_ now folds inline into the loose outline at its marker's position via a new
  **gather subscription** — the count+slice flattener run worker-side against the live Database:
  ...
  - **The materialized source is the _filtered_ outline query.** `computeSegments`/`gatherWindow`
...
in-progress`) owns a real child labelled _"A real child, homed on the task above…"_. That same
...
>   a `view` block, in whatever branch that block lives) may have its _real_ position somewhere
