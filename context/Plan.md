...
5. **Face affinity for matrixes.** When you drill into an aspect/record row or an embedded matrix, which face should render it? Currently defaults to the table identity face. A "preferred face" annotation on a matrix (or attachment) would let plugins specify a more appropriate default. _To be resolved in [Phase 9 §9.5](Phase-9.md)_ (boundary-hop rendering), where the far-side face is decided. _Floor answer from [Phase 9.2](Phase-9.2.md):_ the **substrate** (the identity face generalized to a fidelity) is the universal fallback — with no declared preferred face, drill-in lands in the substrate. #5 narrows to "what preferred face, if any, overrides that default." _Confirmed under the [Phase 9.7 convergence](Phase-9.7.md#9-open-decisions-and-their-resolutions):_ v1 fidelity is **substrate-first** (one renderer; composed faces are [Phase 10](Phase-10.md)), and boundary-hop drill-in is now a **multi-location position** whose breadcrumb is the lexkey-prefix ancestry of the appearance drilled from — so the substrate floor still stands and the narrowed question (which preferred face overrides) is unchanged.
