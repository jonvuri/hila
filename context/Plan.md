...
> Follow-on exploration: Phase 7b (overlaid-cards polish, [Phase-7b.md](Phase-7b.md)) and Phase 7c (row–table continuum design exploration, [Phase-7c.md](Phase-7c.md)). The 7c exploration was promoted into the implementation phases below: its data layer is Phase 8, its view layer is Phase 9, and the design-system pass that follows is Phase 10.

---

### Phase 8 -- Data layer: the ownership spine

> Detailed task list: [Phase-8.md](Phase-8.md) (umbrella + Part A), [Phase-8b.md](Phase-8b.md) (derived projections), [Phase-8c.md](Phase-8c.md) (matrix ownership + tags-as-nodes)

The data-layer realization of the [Phase 7c](Phase-7c.md) ownership-centric resolution. The largest structural change since the foundation: it promotes the `own`-join from "a cross-matrix lifecycle link" to **the universal tree edge**, dissolves the per-matrix `rank` trait (order moves onto the edge as a sibling-local key), rebuilds `closure` and the scroll index as **derived global caches** over the own-forest, unifies cascade deletion, and reduces tags to nodes + ownership. Split into three sequential, session-sized parts:
...
> Detailed task list: [Phase-9.md](Phase-9.md)
...
3. **Singleton tags and shared aspects.** _Resolved by the [Phase 7c](Phase-7c.md) ownership resolution ([Phase 8c](Phase-8c.md)): create-vs-link **is** own-vs-ref._ `#`-tagging that creates a new aspect row is an `own`-edge; tagging/linking an existing entity (and the no-schema label case) is a `ref`-edge. The model now expresses both cleanly via the two edge kinds, so this is no longer open. _Still holds under the [Phase 9.7 convergence](Phase-9.7.md#5-portals-and-refs-one-family-split-by-anchoring):_ `#`/`@` keep their 8c semantics (because ownership stays where-created); the convergence only *adds* a third non-owning relation — the structural **portal** — completing the ownership × anchoring square without disturbing create-vs-link.
...
5. ~~**Face affinity for matrixes.**~~ **Resolved ([Phase 10 §3](Phase-10.md#3-plugin-view-composition-model), session 3):** the **preferred face is a subject-level (matrix-level) rendering recipe** — rung 2 of the composition **affinity ladder** (appearance override [modeled but unstored in v1] → subject/matrix preferred recipe → substrate floor). It is set by the plugin at creation or by the user through panel/block chrome, and rides the membership plane so it follows a container everywhere it appears. The **substrate remains the universal fallback** (rung 3), confirming the [Phase 9.2](Phase-9.2.md) floor answer and the [Phase 9.7](Phase-9.7.md#9-open-decisions-and-their-resolutions) substrate-first stance. See [Plugins.md — the affinity ladder](Plugins.md#the-affinity-ladder-resolves-which-face-renders-a-subject). _(Original framing: when you drill into an aspect/record row or an embedded matrix, which face should render it? A "preferred face" annotation on a matrix would let plugins specify a default over the table identity face.)_
