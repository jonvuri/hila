...
> **Session 3b-i pivot + completion.** The _shared authoring gestures_ item proved to be the foundation the rest stands on, so the session pivoted to settle **the query spec and its SQL-analog gestures** first. The decided model — SQL-canonical + dialect recognizer, the six-dimension spec, escalation tiers, the leaf taxonomy (incl. the CTE source chip), the chip interaction grammar, and the v1 glyph vocabulary — is captured in [Query-Spec.md](Query-Spec.md) (determinations D1–D19), with the visual companion + round-by-round reasoning at [Phase-10-Session-3b-visuals.html](Phase-10-Session-3b-visuals.html). The launcher _surface_ proper is **session 3b-ii**, consuming that model. Chip palette + the opacity/invalid frame treatments are recorded as placeholders whose final form is the [§4](#4-cohesive-design-token-and-theming-system) token pass's.
...
**Next sequence:** [Sessions 4j–4m](Phase-10-Sessions-4j-4m-plan.md) define and implement the
canonical token contract, then close two navigation design gates before live migration. Session 4l
fixes sticky-header threshold motion. Session 4m integrates outline affordances as an independent
component variant. [Sessions 4n–4o](Phase-10-Sessions-4n-4o-plan.md) then migrate the live workspace
and remove the executable overlaid-card implementation.
...
- [x] **Theming model.** Ghost, Null, and Wipeout all ship. Visual theme, dark/light polarity,
      ...
      `data-theme` attribute remains the polarity switch. The next token session will name and
      implement the visual-theme override mechanism.
...
- [ ] **Session 4m:** integrate the old and new navigation-outline variants through a separate
      configuration axis.

### Planned workspace migration and retirement sessions

- [ ] **Session 4n:** migrate the approved workspace into the live shell without changing workspace
      data or gestures.
- [ ] **Session 4o:** remove the executable overlaid-card renderer, variants, styles, stories, and
