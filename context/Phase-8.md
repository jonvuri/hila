...
- [x] **Update `matrix_traits`.** No production code provisions `rank` any more (workspace face `traitRequirements` + plugin `traits` declare only `closure`; `ensureRootMatrix` provisions only `closure`; `createTagType` no longer provisions `rank`; `createTreePosition` no longer calls `requireTraits`). `closure` provisioning left in place (vestigial) until 8b. **Deliberately deferred:** the trait system is removed *wholesale* in [Phase 8b](Phase-8b.md) (`matrix_traits`, the `TraitType` union, `ensureTrait`), so to avoid churn that 8b would redo, `'rank'` is left as a still-registrable (but unused-by-production) `TraitType` value and `matrix_traits` CHECK option for Part A. "The rank trait is gone" in the done criteria means: the `rank` *table* is gone and `rank` is no longer provisioned as a structural trait -- order lives on the edge; the lingering enum value is cleaned up in 8b. The `trait_type IN ('rank', 'closure')` CHECK and `ensureTrait`/`hasTrait`/`requireTraits` usages: `rank` is no longer a provisionable trait (the own-forest is universal infrastructure, not a per-matrix opt-in). `closure` becomes global (Phase 8b) rather than per-matrix, so the trait registry's role shrinks further -- decide in 8b whether `matrix_traits` survives at all or collapses entirely. For this part: stop provisioning/requiring `rank`; leave `closure` provisioning in place until 8b reworks it.
- [x] **Update `src/core/sync.ts` triggers.** Removed `rank` from tracked core tables and added `edge_key` to the tracked `joins` columns; removed the rank-collision and closure-rebuild logic from `applyRemoteChanges` (BLOB decode now applies to `joins.edge_key`). **Replica-stable `joins` apply (composite key, not rowid):** because `joins` now carries the own-forest structure and `reparentRow` *updates* it (changing the own-edge's `source`, part of the composite PK), applying remote `joins` changes by SQLite `rowid` would target the wrong/no row on another replica. So the `joins` DELETE trigger records the OLD row (its logical key), remote `joins` UPDATEs apply as an upsert (`INSERT OR REPLACE` re-homes the own-edge via the `joins_single_owner` partial-unique conflict, keyed by the invariant child/target), and remote `joins` DELETEs apply by the composite `(source, target)` key. (Closure remains untracked/derived -- unchanged principle.)
